...
export function PlayerCard({ name, rating, clock, active }: Props) {
  const { t } = useLanguage();
  return (
    <section className={`flex items-center justify-between rounded-lg p-4 ${active ? "bg-primary-fixed/25" : "bg-surface-container-low"}`}>
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm font-semibold text-on-surface-variant">{t("rating", { value: rating })}</p>
        </div>
      </div>
