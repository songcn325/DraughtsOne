...
export function OnlineGamePage() {
  ...
  async function copyPdn() {
    const copied = await copyText(pdnText);
        ...
        <section className="rounded-lg bg-surface-container-lowest p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <button type="button" onClick={() => {
              setCopyStatus("idle");
              setPdnDialogOpen(true);
            }} className="rounded-full bg-surface-container-low px-4 py-2 text-sm font-black text-primary">
              {t("exportMoves")}
            </button>
          </div>
      ...
      {pdnDialogOpen && (
        <div className="fixed inset-0 z-[70] grid place-items-end bg-black/25 p-4 sm:place-items-center">
          <section className="w-full max-w-lg rounded-lg bg-surface p-6 shadow-[0_24px_80px_rgba(45,47,47,0.2)]">
            ...
            {copyStatus !== "idle" && (
              <p className={`mt-3 font-bold ${copyStatus === "copied" ? "text-primary" : "text-error"}`}>
                {t(copyStatus === "copied" ? "copiedToClipboard" : "copyFailed")}
              </p>
            )}
            <div className="mt-5 grid grid-cols-2 gap-3">
              <TactileButton onClick={() => void copyPdn()}>{t(copyStatus === "copied" ? "copied" : "copy")}</TactileButton>
